 ...
 */
class MainMusicsRecAdapter(private val callBack: MainMusicAdapterCallBack) :
    ...
    inner class MainMusicsRecViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun onBind(song: Song, position: Int) {
            ...
            itemView.setOnClickListener {
                callBack.onMusicItemClicked(song, position)
            }
        }
    }
    ...
    fun setMusicsList(listMusic: List<Song>) {
        musics.clear()
        musics.addAll(listMusic)
        notifyDataSetChanged()
    }

    //interface for notifies the main fragment.
    interface MainMusicAdapterCallBack {
        fun onMusicItemClicked(song: Song, position: Int)
    }
}